...
def main(listen_socket, communicator):
    ...
    while True:
        try:
            ...
            elif command == "protocol" and kwargs["subcommand"] in ["absolute", "additive_inverse", "floor", "logical_not", "less_than_zero", "multiplicative_inverse", "relu", "sum", "truncate", "verify", "zigmoid"]:
                protocol = protocol_stack[-1]
                a = operand_stack.pop()
            ...
            elif command == "protocol" and kwargs["subcommand"] in ["add", "divide", "dot", "equal", "less", "logical_and", "logical_or", "logical_xor", "max", "min", "multiply", "private_public_power", "subtract", "untruncated_divide", "untruncated_multiply"]:
                protocol = protocol_stack[-1]
                b = operand_stack.pop()
                a = operand_stack.pop()
