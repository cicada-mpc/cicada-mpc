...
def main(listen_socket, communicator):
    ...
    while True:
        try:
            ...
            elif command == "protocol" and kwargs["subcommand"] in ["absolute", "additive_inverse", "bit_compose", "bit_decompose", "floor", "logical_not", "less_zero", "multiplicative_inverse", "relu", "sum", "truncate", "verify", "zigmoid"]:
                protocol = protocol_stack[-1]
                a = operand_stack.pop()
            ...
            elif command == "protocol" and kwargs["subcommand"] in ["field_add", "divide", "dot", "equal", "less", "logical_and", "logical_or", "logical_xor", "max", "min", "multiply", "private_public_power", "private_public_power_field", "field_subtract", "untruncated_divide", "untruncated_multiply"]:
                protocol = protocol_stack[-1]
                b = operand_stack.pop()
                a = operand_stack.pop()
