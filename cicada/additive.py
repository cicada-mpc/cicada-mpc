...
class AdditiveProtocolSuite(object):
    ...
    def less(self, lhs, rhs):
        ...
        twolhs = self.field_multiply(self.field(2), lhs)
        tworhs = self.field_multiply(self.field(2), rhs)
        diff = self.field_subtract(lhs, rhs)
        twodiff = self.field_multiply(self.field(2), diff)
        w = self.field_subtract(self.field(1), self._lsb(twolhs))
        x = self.field_subtract(self.field(1), self._lsb(tworhs))
        y = self.field_subtract(self.field(1), self._lsb(twodiff))
        wxorx = self.logical_xor(w,x)
        notwxorx = self.field_subtract(self.field(1), wxorx)
        xwxorx = self.field_multiply(x, wxorx)
        noty = self.field_subtract(self.field(1), y)
