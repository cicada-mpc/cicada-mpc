...
class AdditiveSharing(object):
    ...
    def reveal(self, share, dst=None, encoding=None):
        ...
        for recipient in dst:
            received_shares = self._communicator.gather(value=share.storage, dst=recipient)

            # If we're a recipient, recover the secret.
            if self._communicator.rank == recipient:
    ...
    def share(self, *, src, secret, shape, encoding=None):
        """Convert an array into an additive secret share.
        ...
        if self._communicator.rank == src:
        ...
        if self._communicator.rank == src:
            self._field.inplace_add(przs, self._field(encoder.encode(secret)))
