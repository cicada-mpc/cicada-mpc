...
def main(communicator):
    ...
    b_enc = protocol.encoder.encode(numpy.array(b))

    log.info(f"Player {communicator.rank} secret a: {secret_a}", src=0)
    log.info(f"Player {communicator.rank} b: {b}", src=1)

    a_share = protocol.share(src=0, secret=protocol.encoder.encode(secret_a), shape=())#secret=protocol.encoder.encode(secret_a), shape=())
