...
def main(communicator):
    ...
    shamir = cicada.shamir.ShamirProtocol(communicator, threshold=3, modulus=11311, precision=2)
    ...
    log.info(f"Player {communicator.rank} built vandermonde:\n{vandermonde(numpy.array([1,2,3,4,5]))}", src=0)
    original_coef = numpy.linalg.solve(vandermonde(numpy.array([1,2,3,4,5])), numpy.array([12640, 32228, 58768, 92260, 132704]))
    log.info(f"Player {communicator.rank} linalg solve:\n{original_coef}", src=0)
    log.info(f"Player {communicator.rank} built vandermonde:\n{numpy.vander(numpy.array([1,2,3,4,5]), increasing=True)}", src=0)
